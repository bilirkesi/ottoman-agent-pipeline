...
class KeyVault:
    ...
    async def create_key(
        ...
        meta = KeyMetadata(
            ...
            metadata=metadata or {"scope_id": scope_id, "created_by": "byok_system"},
        )
        ...
        # Audit
        self._add_audit_log(action="create", key_id=key_id, success=True)
    ...
    async def get_key(self, key_id: str, user_id: str | None = None) -> str | None:
        """
        ...
        # Decrypt
        try:
            plaintext = self._decrypt(record.encrypted_key, record.iv, record.auth_tag)
            ...
            # Audit
            self._add_audit_log(
                action="use", key_id=key_id, user_id=user_id, success=True
            )
    ...
    async def rotate_key(
        self, key_id: str, new_api_key: str, user_id: str | None = None
    ) -> bool:
        """
        Anahtar rotasyonu
        """
        ...
        # Audit
        self._add_audit_log(
            action="rotate", key_id=key_id, user_id=user_id, success=True
        )
    ...
    async def revoke_key(self, key_id: str, user_id: str | None = None) -> bool:
        """
        Anahtar iptal
        """
        if key_id not in self.keys:
        ...
        # Audit
        self._add_audit_log(
            action="revoke", key_id=key_id, user_id=user_id, success=True
        )
            ...
            results.append(
                {
                    ...
                    "expires_at": (
                        record.metadata.expires_at.isoformat()
                        if record.metadata.expires_at
                        else None
                    ),
                    "usage_count": record.metadata.usage_count,
                    "last_used_at": (
                        record.metadata.last_used_at.isoformat()
                        if record.metadata.last_used_at
                        else None
                    ),
                }
            )
    ...
    async def get_audit_logs(
        self, key_id: str | None = None, action: str | None = None, limit: int = 100
    ) -> list[dict]:
        """
    ...
    async def cleanup_expired_keys(self) -> int:
        ...
        for key_id, record in list(self.keys.items()):
            if (
                ...
                and datetime.now() > record.metadata.expires_at
            ):
                record.status = KeyStatus.EXPIRED
                cleaned += 1
    ...
    async def get_stats(self) -> dict:
        ...
        return {
            ...
            "services": list({k.metadata.service for k in self.keys.values()}),
        }
    ...
    def _load(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self.keys)} keys, {len(self.audit_logs)} audit logs"
            )
    ...
    def _save(self):
        ...
        try:
            data = {
                ...
                "audit_logs": [
                    log.to_dict() for log in self.audit_logs[-1000:]
                ],  # Keep last 1000
                "metadata": {
                    "version": "1.0.0",
            ...
            logger.debug(
                f"Saved {len(self.keys)} keys, {len(self.audit_logs)} audit logs"
            )
...
class DeepSeekKeyManager:
    ...
    async def create_key(self, api_key: str, **kwargs) -> str:
        return await self.vault.create_key(
            service=self.service, api_key=api_key, **kwargs
        )
...
def get_keyvault(master_key: str | None = None) -> KeyVault:
