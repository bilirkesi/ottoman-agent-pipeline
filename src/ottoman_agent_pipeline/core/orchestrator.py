...
class AgentOrchestrator:
    ...
    def __init__(
        self, config: AgentConfig | None = None, session_id: str | None = None
    ):
    ...
    async def initialize(self) -> None:
        ...
        logger.info(
            f"Agent initialized with {len(self.tools)} tools and {len(self.models)} models"
        )
    ...
    async def _initialize_tools(self) -> None:
        ...
        if tool_configs.ner.enabled:
            self.tools["ner"] = NERTool(model=tool_configs.ner.model)
    ...
    async def _initialize_models(self) -> None:
        ...
        if "reasonix" in model_configs.providers:
            ...
            self.models["reasonix"] = ReasonixModel(
                api_key=reasonix_config.api_key, base_url=reasonix_config.base_url
            )

        logger.info(f"Initialized {len(self.models)} model providers")

    async def chat(
        self, message: str, model: str | None = None, stream: bool = False, **kwargs
        ...
        try:
            ...
            system_prompt = self._prompt_builder.build(
                tools=list(self.tools.keys()), models=list(self.models.keys())
            )
    ...
    async def _stream_response(
        self, model_provider: ModelProvider, messages: list[dict[str, str]], **kwargs
    ...
    async def _execute_tool_calls(
        self, tool_calls: list[dict[str, Any]], current_output: str
    ) -> str:
    ...
    def _build_messages(self, message: str) -> list[dict[str, str]]:
        ...
        history = self.session.get_history(limit=self.config.sessions.max_history)
    ...
    def _get_fallback_model(self, current_model_name: str) -> ModelProvider | None:
    ...
    async def translate(
        self, text: str, mode: str = "hybrid", model: str | None = None
    ) -> AgentResponse:
        """
    ...
    async def analyze(
        ...
        """
        prompt = "Analyze this Ottoman Turkish text:"
        if entities:
