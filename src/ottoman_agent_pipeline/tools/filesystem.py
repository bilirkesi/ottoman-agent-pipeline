...
class FileSystemTool(BaseTool):
    ...
    async def execute(
        self, action: str, path: str = "", content: str = "", **kwargs
    ) -> Any:
        """
    ...
    async def _list_directory(
        self, path: Path, content: str = "", **kwargs
    ) -> list[dict[str, Any]]:
        """List directory contents."""
        if not path.exists():
        ...
        for item in path.iterdir():
            ...
            if recursive and item.is_dir() and max_depth > 1:
                sub_entries = await self._list_directory(item, max_depth=max_depth - 1)
                entries.extend(sub_entries)

        return entries

    async def _search_files(
        self, path: Path, content: str = "", pattern: str = "*", **kwargs
    ) -> list[str]:
