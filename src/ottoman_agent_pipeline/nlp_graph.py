...
class CharacterGraph:
    ...
    def _init_mappings(self):
        ...
        mappings = {
            "ا": [{"text": "a", "confidence": 0.95, "context": "initial"},
            ...
            "ع": [{"text": "a", "confidence": 0.85},
                   {"text": "ə", "confidence": 0.80}],
            ...
            "و": [{"text": "v", "confidence": 0.90},
                   {"text": "o", "confidence": 0.85},
                   {"text": "u", "confidence": 0.85}],
            "ه": [{"text": "h", "confidence": 0.95},
                   {"text": "e", "confidence": 0.90}],
            "ء": [{"text": "", "confidence": 0.80}],
            "ء": [{"text": "'", "confidence": 0.70}],
            "ي": [{"text": "y", "confidence": 0.95},
                   {"text": "i", "confidence": 0.90}],
            "ى": [{"text": "ı", "confidence": 0.95},
                   {"text": "i", "confidence": 0.90}],
            "ئ": [{"text": "e", "confidence": 0.85},
                   {"text": "ə", "confidence": 0.80}],
        }
    ...
    def add_char_mapping(self, arabic_char: str, variants: List[Dict]):
        ...
        for i, variant in enumerate(variants):
            ...
            self.graph.add_node(variant_id,
                               type="variant",
                               text=variant["text"],
                               confidence=variant.get("confidence", 0.9))
            self.graph.add_edge(node_id, variant_id,
                               type="map",
        ...
        for i in range(n):
            ...
            if char in self.char_mappings:
                ...
                for variant in variants:
                    ...
                    if i + 1 not in dp or new_confidence > dp[i + 1].get("confidence", 0):
                        dp[i + 1] = {
                            "path": new_path,
    ...
    def get_alternatives(self, char: str, limit: int = 5) -> List[Dict]:
        ...
        variants = self.char_mappings[char]
        return sorted(variants, key=lambda v: v.get("confidence", 0), reverse=True)[:limit]
    ...
    def get_graph_stats(self) -> Dict:
        ...
        return {
            ...
            "node_types": dict(self.graph.nodes(data="type")) if self.graph.nodes else {}
        }
...
class WordGraph:
    ...
    def add_word(self, word: str, root: str, pos: str, 
                suffixes: List[str] = None, confidence: float = 0.9):
        """Kelime ekle"""
        ...
        self.graph.add_node(root_id,
                           ...
                           text=root,
                           pos=pos)
        self.graph.add_edge(node_id, root_id,
                           type="derives_from",
                           weight=confidence)
    ...
    def get_graph_stats(self) -> Dict:
        ...
        return {
            "total_nodes": self.graph.number_of_nodes(),
            "total_edges": self.graph.number_of_edges(),
            "words": len([n for n, d in self.graph.nodes(data=True) if d.get("type") == "word"]),
            "roots": len([n for n, d in self.graph.nodes(data=True) if d.get("type") == "root"])
        }
...
class EntityGraph:
    ...
    def __init__(self):
        self.graph = nx.Graph()
        self.entity_types = {"PERSON", "LOCATION", "ORGANIZATION", "DATE", "EVENT", "TITLE"}
    ...
    def find_entity_path(self, entity1_id: str, entity2_id: str) -> Optional[List[str]]:
...
class DocumentGraph:
    ...
    def add_paragraph(self, para_id: str, text: str,
                     topic: str = "", embeddings: List[float] = None):
        """Paragraph ekle"""
    ...
    def add_sentence(self, sent_id: str, text: str,
                    para_id: str, pos_tags: List[str] = None):
        """Sentence ekle"""
        self.graph.add_node(sent_id,
                           type="sentence",
                           text=text,
    ...
    def get_flow_analysis(self) -> Dict:
        ...
        para_ids = [pid for pid, data in self.graph.nodes(data=True) 
    ...
    def get_graph_stats(self) -> Dict:
        ...
        return {
            ...
            "total_sentences": sum(p.get("sentence_count", 0) for p in self.paragraphs.values())
        }
...
class NLPGraph:
    ...
    async def add_entity(self, text: str, entity_type: str,
                        start: int = 0, end: int = 0,
                        confidence: float = 0.9) -> str:
        """Entity ekle"""
        return self.entity_graph.add_entity(text, entity_type, start, end, confidence)
    
    async def add_relationship(self, entity1_id: str, entity2_id: str,
                              relationship: str, confidence: float = 0.8):
        """İlişki ekle"""
        self.entity_graph.add_relationship(entity1_id, entity2_id, relationship, confidence)
        
        # Cross-graph connection
        self.cross_graph.add_edge(entity1_id, entity2_id, type=relationship)
    
    async def add_paragraph(self, para_id: str, text: str,
                           topic: str = "", embeddings: List[float] = None):
        """Paragraph ekle"""
        self.doc_graph.add_paragraph(para_id, text, topic, embeddings)
    
    async def add_sentence(self, sent_id: str, text: str,
                          para_id: str, pos_tags: List[str] = None):
        """Sentence ekle"""
    ...
    async def save(self):
        ...
        data = {
            ...
            "word_graph": {
                "words": self.word_cache
            },
            "entity_graph": {
                "entities": dict(self.entity_graph.word_cache)
            }
        }
    ...
    async def load(self):
        ...
        try:
            ...
            if "char_graph" in data:
                ...
                self.char_graph.context_rules = data["char_graph"].get("context_rules", {})
