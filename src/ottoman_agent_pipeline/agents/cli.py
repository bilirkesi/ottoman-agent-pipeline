...
@app.command()
def deploy(version: str = typer.Option("0.1.0", help="Sürüm numarası")):
    """Deploy et"""
    orch = get_orchestrator()
...
@app.command()
def research(topic: str = typer.Argument(..., help="Araştırma konusu")):
    """Araştırma yap"""
    orch = get_orchestrator()
...
@app.command()
def document(project: str = typer.Option("ottoman-agent-pipeline", help="Proje adı")):
        ...
        pipeline = [
            {
                "name": "Research",
                "agent": "research_agent",
                "payload": {"action": "research", "topic": "Ottoman NLP"},
            },
            {
                "name": "Implement",
                "agent": "code_agent",
