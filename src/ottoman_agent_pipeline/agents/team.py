...
class AgentBus:
    ...
    async def send(self, sender: str, receiver: str, payload: dict):
        ...
        message = AgentMessage(
            sender=sender, receiver=receiver, msg_type="request", payload=payload
        )
...
class Agent:
    ...
    async def handle(self, message: AgentMessage) -> dict | None:
...
class ResearchAgent(Agent):
    ...
    async def research(self, config: dict) -> dict:
        ...
        return {"topic": config["topic"], "findings": analysis, "sources": results}
...
class ProjectOrchestrator:
    ...
    async def run_task(self, task: dict) -> dict:
        ...
        result = await self.bus.send(
            sender="orchestrator", receiver=agent_name, payload=payload
        )
    ...
    async def run_pipeline(self, pipeline: list[dict]) -> dict:
        ...
        return {
            "status": (
                "success"
                if all(r.get("status") != "failed" for r in results)
                else "failed"
            ),
            "tasks": len(results),
            "results": results,
