...
class MCPToolRegistry:
    ...
    async def execute_tool(
        ...
        if not await self._check_rate_limit(tool_id):
            return {"success": False, "error": "Rate limit exceeded", "duration_ms": 0}
    ...
    async def _execute(
        self, config: ToolConfig, params: dict, key_id: str | None
    ) -> Any:
        """Tool execute"""
    ...
    async def _handle_filesystem(self, params: dict, key_id: str | None) -> dict:
        """Filesystem handler"""
    ...
    async def _handle_web_search(self, params: dict, key_id: str | None) -> dict:
        """Web search handler"""
    ...
    async def _handle_translation(self, params: dict, key_id: str | None) -> dict:
        """Translation handler"""
    ...
    async def _handle_ner(self, params: dict, key_id: str | None) -> dict:
        """NER handler"""
    ...
    async def _check_rate_limit(self, tool_id: str) -> bool:
        ...
        self.rate_limits[tool_id] = [
            t for t in self.rate_limits[tool_id] if t > one_hour_ago
        ]
    ...
    async def get_stats(self) -> dict:
        ...
        avg_duration = sum(c.duration_ms for c in self.call_history) / max(
            total_calls, 1
        )
    ...
    def _load(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self.tools)} tools, {len(self.call_history)} call history"
            )
    ...
    def _save(self):
        ...
        try:
            data = {
                ...
                "call_history": [
                    call.to_dict() for call in self.call_history[-1000:]
                ],  # Keep last 1000
                "metadata": {
                    "version": "1.0.0",
