...
class CodeGraph:
    ...
    async def get_dependencies(self, node_id: str) -> List[Tuple[str, str, float]]:
        ...
        for neighbor in self.graph.successors(node_id):
            ...
            if edge_data.get('type') in ['imports', 'depends_on']:
                deps.append((neighbor, edge_data.get('type'), edge_data.get('weight', 1.0)))
        return deps
    ...
    async def get_orphan_nodes(self) -> List[str]:
        ...
        for node_id in self.nodes:
            if not self.graph.has_predecessors(node_id) and not self.graph.has_successors(node_id):
    ...
    async def _load(self) -> None:
        ...
        if db_file.exists():
            try:
                ...
                logger.info(f"Loaded {len(self.nodes)} nodes, {len(self.edges)} edges from {db_file}")
    ...
    async def _save(self) -> None:
        ...
        try:
            ...
            logger.debug(f"Saved {len(self.nodes)} nodes, {len(self.edges)} edges to {self.db_path}")
